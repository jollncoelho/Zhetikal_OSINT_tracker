...
export const HermesAnalyzer: React.FC<{ targetValue?: string; targetType?: string }> = ({ 
  targetValue = "Ziggy", 
  targetType = "Username" 
}) => {
  ...
  const triggerJarvisAnalysis = async () => {
    ...
    const context: HermesMessage[] = [
      ...
      { role: 'user', content: `Donne 3 pivots pour : ${targetValue} (${targetType})` }
    ];
  ...
  return (
    <div className="w-full max-w-xl p-5 bg-black border border-purple-900/50 rounded-xl font-mono text-sm shadow-2xl my-4">
      ...
      <button 
        ...
        className="w-full py-2.5 px-4 rounded-lg font-bold bg-purple-900/20 text-purple-300 border border-purple-700/60 hover:bg-purple-800/40"
      >
